    ...
    (function(window2, document2, exportName, undefined2) {
      ...
      function computeIntervalInputData(session, input) {
        ...
        if (input.eventType != INPUT_CANCEL && (deltaTime > COMPUTE_INTERVAL || last2.velocity === undefined2)) {
          ...
          velocityX = v.x;
          velocityY = v.y;
          velocity = abs8(v.x) > abs8(v.y) ? v.x : v.y;
...
function createTouchRecognizer(node) {
  function stopMouse(event2) {
    forEach(mouseEvents, function(e2) {
      event.bind(node, e2, stopEvent, true);
    });
  }
  function allowMouse(event2) {
    setTimeout(function() {
      forEach(mouseEvents, function(e2) {
        event.unbind(node, e2, stopEvent, true);
      });
    }, 500);
  }
...
function TouchInteractionEvents(injector, canvas, eventBus, elementRegistry, interactionEvents) {
  ...
  function initEvents(svg) {
    ...
    function startZoom(e2) {
      ...
      var zoom2 = canvas.zoom(), mid4 = e2.center;
      function update(e3) {
...
PopupMenu.prototype.close = function() {
  ...
  remove2(this._current.container);
...
PopupMenu.prototype.trigger = function(event2) {
  event2.preventDefault();
  var element = event2.delegateTarget || event2.target, entryId = attr2(element, DATA_REF);
...
PopupMenu.prototype._createContainer = function(id) {
  ...
  classes2(container).add(className);
  attr2(container, "data-popup", id);
  return container;
};
PopupMenu.prototype._attachContainer = function(container, parent, cursor) {
  ...
  assign2(container, { visibility: "visible" });
};
...
PopupMenu.prototype._assureIsInbounds = function(container, cursor) {
  ...
  assign2(container, { left, top }, { "zIndex": 1e3 });
};
PopupMenu.prototype._createEntries = function(entries, className) {
  var entriesContainer = domify$1("<div>"), self2 = this;
  classes2(entriesContainer).add(className);
  forEach(entries, function(entry, id) {
    var entryContainer = self2._createEntry(entry, id), grouping = entry.group || "default", groupContainer = query("[data-group=" + (0, import_css.default)(grouping) + "]", entriesContainer);
    if (!groupContainer) {
      groupContainer = domify$1('<div class="group"></div>');
      attr2(groupContainer, "data-group", grouping);
...
PopupMenu.prototype._createEntry = function(entry, id) {
  var entryContainer = domify$1("<div>"), entryClasses = classes2(entryContainer);
  ...
  attr2(entryContainer, DATA_REF, id);
  if (entry.label) {
...
function setTransform(element, transform2) {
  ...
  ["", "-ms-", "-webkit-"].forEach(function(prefix) {
    element.style[prefix + "transform"] = transform2;
  });
}
...
function getClosestSegment(position, connection) {
  ...
  for (var i = 0; i < waypoints.length - 1; i++) {
    var start = waypoints[i], end = waypoints[i + 1], distance = getDistancePointLine(position, [start, end]);
...
function BendpointSnapping(eventBus) {
  ...
  eventBus.on([
    ...
    if (!isConnection2(hover) || !hoverMid || !hoverMid.x || !hoverMid.y) {
      return;
    }
...
TextBox.prototype.resizable = function() {
  ...
  if (!resizeHandle) {
    ...
    var onMouseMove = function(e2) {
      preventDefault2(e2);
      stopPropagation2(e2);
      var newWidth = min(max2(startWidth + e2.clientX - startX, minWidth), maxWidth);
      var newHeight = min(max2(startHeight + e2.clientY - startY, minHeight), maxHeight);
...
function isSingleShape(elements) {
  return elements && elements.length === 1 && !isConnection3(elements[0]);
}
...
function BpmnCopyPaste(bpmnFactory, eventBus, moddleCopy) {
  ...
  eventBus.on("copyPaste.pasteElement", LOW_PRIORITY6, function(context) {
...
function DistributeElementsMenuProvider(popupMenu, distributeElements, translate2, rules) {
  ...
  this._popupMenu = popupMenu;
  this._rules = rules;
  popupMenu.registerProvider("align-elements", LOW_PRIORITY7, this);
}
...
var LOW_PRIORITY8 = 800;
function GridSnapping(elementRegistry, eventBus, config) {
  var active = !config || config.active !== false;
  this._eventBus = eventBus;
  var self2 = this;
  eventBus.on("diagram.init", LOW_PRIORITY8, function() {
...
function ResizePreview(eventBus, canvas, previewSupport) {
  ...
  eventBus.on("resize.move", LOW_PRIORITY9, function(event2) {
...
ResizeHandles.prototype.addResizer = function(element) {
  var self2 = this;
  if (isConnection5(element) || !this._resize.canResize({ shape: element })) {
    return;
  }
...
function CreateParticipantBehavior(canvas, eventBus, modeling) {
  ...
  eventBus.on([
    ...
    var children = rootElement.children.filter(function(element) {
      return !is(element, "bpmn:Group") && !isLabel(element) && !isConnection6(element);
    });
    if (!children.length) {
      return;
    }
...
function fenced(n, rangeStart, rangeEnd) {
  return n >= min3(rangeStart, rangeEnd) - EQUAL_THRESHOLD && n <= max4(rangeStart, rangeEnd) + EQUAL_THRESHOLD;
}
function mid2(p1, p2) {
  return {
    x: (p1.x + p2.x) / 2,
...
function pointsEqual(p1, p2) {
  return abs4(p1.x - p2.x) <= EQUAL_THRESHOLD && abs4(p1.y - p2.y) <= EQUAL_THRESHOLD;
}
...
function SubProcessPlaneBehavior(canvas, eventBus, modeling, elementFactory, bpmnFactory, bpmnjs, elementRegistry) {
  ...
  this.executed("shape.toggleCollapse", LOW_PRIORITY14, function(context) {
    var shape = context.shape;
    if (!is(shape, "bpmn:SubProcess")) {
      return;
    }
  ...
  this.reverted("shape.toggleCollapse", LOW_PRIORITY14, function(context) {
    var shape = context.shape;
    if (!is(shape, "bpmn:SubProcess")) {
      return;
    }
...
function ToggleElementCollapseBehaviour(eventBus, elementFactory, modeling, resize) {
  ...
  this.executed(["shape.toggleCollapse"], LOW_PRIORITY15, function(e2) {
  ...
  this.reverted(["shape.toggleCollapse"], LOW_PRIORITY15, function(e2) {
  ...
  this.postExecuted(["shape.toggleCollapse"], LOW_PRIORITY15, function(e2) {
...
function UpdateFlowNodeRefsBehavior(eventBus, modeling, translate2) {
  ...
  this.postExecuted(laneRefUpdateEvents, LOW_PRIORITY16, function(event2) {
    releaseContext();
  });
  this.preExecute([
    "shape.create",
...
BpmnRules.prototype.init = function() {
  ...
  this.addRule("elements.create", function(context) {
    ...
    if (isConnection7(target) && !canInsert(elements, target, position)) {
      return false;
    }
...
function getSpaceToolConstraints(elements, axis, direction, start, minDimensions) {
  ...
  forEach(resizingShapes, function(resizingShape) {
    ...
    var nonMovingResizingChildren = filter(children, function(child) {
      return !isConnection8(child) && !isLabel5(child) && !includes6(movingShapes, child) && !includes6(resizingShapes, child);
    });
    var movingChildren = filter(children, function(child) {
      return !isConnection8(child) && !isLabel5(child) && includes6(movingShapes, child);
    });
  ...
  eventBus.on("spaceTool.move", LOW_PRIORITY18, function(event2) {
    ...
    if (!context.dragGroup) {
      ...
      var movingConnections = context.movingConnections = elementRegistry.filter(function(element) {
        ...
        return isConnection9(element) && (sourceIsMoving || sourceIsResizing) && (targetIsMoving || targetIsResizing);
      });
...
function LabelSupport(injector, eventBus, modeling) {
  ...
  eventBus.on("shape.move.start", HIGH_PRIORITY16, function(e2) {
    var context = e2.context, shapes = context.shapes, validatedShapes = context.validatedShapes;
    context.shapes = removeLabels(shapes);
    context.validatedShapes = removeLabels(validatedShapes);
  });
  movePreview && eventBus.on("shape.move.start", LOW_PRIORITY19, function(e2) {
  ...
  this.preExecuted("elements.move", HIGH_PRIORITY16, function(e2) {
...
function AttachSupport(injector, eventBus, canvas, rules, modeling) {
  CommandInterceptor.call(this, eventBus);
  var movePreview = injector.get("movePreview", false);
  eventBus.on("shape.move.start", HIGH_PRIORITY17, function(e2) {
    var context = e2.context, shapes = context.shapes, validatedShapes = context.validatedShapes;
    context.shapes = addAttached(shapes);
    context.validatedShapes = removeAttached(validatedShapes);
  });
  movePreview && eventBus.on("shape.move.start", LOW_PRIORITY20, function(e2) {
  ...
  this.preExecuted("elements.move", HIGH_PRIORITY17, function(e2) {
...
ElementFactory2.prototype.create = function(elementType, attrs) {
  if (elementType === "label") {
    ...
    return this.baseCreate(elementType, assign({ type: "label", di }, DEFAULT_LABEL_SIZE, attrs));
  }
...
SetColorHandler.prototype.postExecute = function(context) {
  ...
  forEach(elements, function(element) {
    var assignedDi = isConnection12(element) ? pick(di, ["border-color"]) : di, elementDi = getDi(element);
...
function MovePreview(eventBus, canvas, styles, previewSupport) {
  ...
  eventBus.on("shape.move.start", LOW_PRIORITY22, function(event2) {
    ...
    forEach(allDraggedElements, function(e2) {
      canvas.addMarker(e2, MARKER_DRAGGING2);
    });
  ...
  eventBus.on("shape.move.cleanup", function(event2) {
    ...
    forEach(allDraggedElements, function(e2) {
      canvas.removeMarker(e2, MARKER_DRAGGING2);
    });
    if (dragGroup) {
      remove(dragGroup);
    }
...
function HandTool(eventBus, canvas, dragging, injector, toolManager, mouse) {
  ...
  keyboard && keyboard.addListener(HIGH_PRIORITY19, function(e2) {
...
ResizeSnapping.prototype.getSnapTargets = function(shape, target) {
  return getChildren(target).filter(function(child) {
    return !isAttached(child, shape) && !isConnection15(child) && !isHidden2(child) && !isLabel7(child);
  });
};
